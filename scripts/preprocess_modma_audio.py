...
Uses subjects_information_audio_lanzhou_2015.xlsx for correct labels (29 HC, 23 MDD).
"""
...
def main():
    ...
    log("  Labels from Excel: 29 HC, 23 MDD")
    log("="*70)

    df_xlsx = pd.read_excel(AUDIO_XLSX)
    log(f"Excel subjects: {len(df_xlsx)} ({df_xlsx['type'].value_counts().to_dict()})")
    ...
    for sd in sub_dirs:
        ...
        if not subj_feats:
            ...
            continue
        keys = sorted(subj_feats[0].keys())
        avg_feat = {k: np.mean([sf[k] for sf in subj_feats]) for k in keys}
    ...
    # Map to labels
    df_xlsx_sorted = df_xlsx.sort_values('subject id')
    excel_labels = df_xlsx_sorted['type'].tolist()
    y = np.array([1 if l == 'MDD' else 0 for l in excel_labels])

    log(f"Labels: {np.sum(y==1)} MDD, {np.sum(y==0)} HC")

    np.savez(OUT_PATH,
             subjects=np.array(all_ids), X=X,
